...
function Start() {
    ...
    for (var i = 0; i < 10; i++) {
        ...
        for (var j = 0; j < 10; j++) {
            ...
            {
                board[i][j] = 4; // 4 in the 2d array is a wall
            }
            else{
            var randomNum = Math.random();
            if (randomNum <= 1.0 * food_remain / cnt) {
                food_remain--;
                board[i][j] = 1; //1 in the 2d array is food
            } else if (randomNum < 1.0 * (pacman_remain + food_remain) / cnt) {
                shape.i=i;
                shape.j=j;
                pacman_remain--;
                board[i][j] = 2; //2 in the 2d array is pacman
            } else {
                board[i][j] = 0; //0 in the 2d array is empty
            }
            cnt--;
            }
        }
...
function Draw() {
    ...
    for (var i = 0; i < 10; i++) {
        for (var j = 0; j < 10; j++) {
            ...
            center.x = i * 60 + 30;
            center.y = j * 60 + 30;
            if (board[i][j] == 2) { //2 in the 2d array is pacman
                context.beginPath();
                ...
                context.fill();
            } else if (board[i][j] == 1) { //1 in the 2d array is food
                context.beginPath();
...
function ShowSection(id)
    {
    	...
    	registertion_div.style.visibility="hidden";
